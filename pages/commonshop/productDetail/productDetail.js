...
Page({
  ...
  getDefault () {
    ...
    utils.$http(baseUrl + '/emallMiniApp/address/getDefault/' + shopId + '/' + storeId, {}).then(res => {
      ...
      if (_rst) {
        if (_rst.region) {
          defAddress = _rst.region
        } else {
          // 省
          defAddress += _rst.province.indexOf('省') == -1 ? `${_rst.province}省` : _rst.province
          defAddress += _rst.city
          defAddress += _rst.circle
        }
        that.setData({
          DefAddress: defAddress,
          AddressAddId: _rst.addId,
          IsWarning: !_rst.area
        })
        that.setData({
          perfectaddr: false
  ...
  Judge() {
    let that = this
    let { supplierSkuId } = that.data
    // 如果不是供应链商品，都不检查
    if (!supplierSkuId) return
    // 判断商品是否支持退货
    that.isSupportNoReasonReturn()
    // 检查供应链商品是否可售
    that.checkSupSkuEnableSale()
  },
  // 判断商品是否支持退货
  isSupportNoReasonReturn() {
    let that = this
    let { supplierSkuId } = that.data
    let data = { supSkuId: supplierSkuId}
    let { storeId, shopId, baseUrl } = app.globalData
    utils.$http(baseUrl + `/emallMiniApp/goods/isSupportNoReasonReturn/${shopId}/${storeId}/${data.supSkuId}`, data).then(res => {
      that.setData({isSupportNoReasonReturn: res.result.result})
    }).catch((res) => { console.log(res) })
  },
  // 获取促销商品
  getSkuGift() {
    let that = this
    let { supplierSkuId, AddressAddId, lat, lng } = that.data
    let { storeId, shopId, baseUrl } = app.globalData
    // 如果不是供应链商品，都不检查
    if (!supplierSkuId) return
    let data = {
      shopId: shopId,
      storeId: storeId,
      supSkuId: supplierSkuId
    }
    if (AddressAddId) {
      data.addId = AddressAddId
    } else {
      data.lat = lat
      data.lng = lng
    }
    ...
    utils.$http(`${baseUrl}/emallMiniApp/goods/skuGift/${shopId}/${storeId}/${data.supSkuId}`, data).then(res => {
      if (res) {
        let _rst = res.result
        that.setData({
          skuGiftList: _rst.skuGiftList
        })
        console.log(_rst)
      }
    }).catch((res) => { })
  ...
  checkSupSkuEnableSale() {
    ...
    if (supplierSkuId) {
      ...
      data.supSkuId = supplierSkuId
      utils.$http(baseUrl + `/emallMiniApp/goods/checkSupSkuEnableSale/${shopId}/${storeId}/${supplierSkuId}`, data).then(res => {
        console.log(res)
        let rest = res.result
        that.setData({
          warningData: !rest.result ? rest: ''
        })
      }).catch((res) => { console.log(res)})
    }
  },
  ...
  getLocation () {
    ...
    wx.getLocation({ success: e => {
      that.setData({ lat: e.latitude, lng: e.longitude })
      // 获取促销商品
      // that.getSkuGift()
      qqmapsdk.reverseGeocoder({
        ...
        success(res) {
          let rst = res.result
          let addcment = rst.address_component
          defAddress += addcment.province
          defAddress += addcment.city
          defAddress += addcment.district
          that.setData({
            DefAddress: defAddress
          })
        },
  ...
  initData() {
      ...
      let defAddress = ''
      if (updateAddr && sendAddObj) {
          if (sendAddObj.region) {
              defAddress = sendAddObj.region
          } else {
              // 省
              defAddress += sendAddObj.province.indexOf('省') == -1 ? `${sendAddObj.province}省` : sendAddObj.province
              defAddress += sendAddObj.city
              defAddress += sendAddObj.circle
          }
          console.log(sendAddObj)
          console.log('前端设置默认地址')
          this.setData({
              DefAddress: defAddress,
              AddressAddId: sendAddObj && sendAddObj.addId,
              IsWarning: !sendAddObj.area,
              perfectaddr: false
          })
          console.log('有默认地址')
      } else {
  ...
  checkShopActivity() { //检查商品是否参与活动
    ...
    utils.$http(baseUrl + '/emallMiniApp/goods/activities/' + shopId + '/' + storeId + '/' + goodsId, {}).then(res => {
      if (res) {
        let _rst = res.result
        let flag = false
        if (_rst) { //代表该商品参加了活动
          if (_rst.activityType === 7) { // 防止分享时为普通商品 点开时已经参与活动 跳到对应活动详情
            flag = true
            wx.redirectTo({
              url: `/pages/secondshop/productDetail/productDetail?activityId=${_rst.activityId}&goodsId=${goodsId}&activityType=${_rst.activityType}&timeStatus=${_rst.timeStatus}`
            })
            return
          } else if (_rst.activityType === 12) { // 抱团
            flag = true
            wx.redirectTo({
              url: `/pages/teamshop/productDetail/productDetail?activityId=${_rst.activityId}&goodsId=${goodsId}&activityType=${_rst.activityType}`
            })
            return
          }
        }
  ...
  gotoOffer(evt) {
    ...
    let params = {
      ...
      skuId: _data.showSku.skuId ? _data.showSku.skuId : _data.skuList[0].skuId,
      shopId: shopId,
      storeId: storeId,
