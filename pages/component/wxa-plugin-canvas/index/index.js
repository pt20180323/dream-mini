    ...
    create(config) {
      ...
      this.initCanvas(config.width, height, config.debug)
        .then(() => {
          ...
          //设置画布背景渐变
          if (config.linearColor) {
            this.ctx.save();
            let grd = this.ctx.createLinearGradient(0, 0, 0, this.toPx(height))
            grd.addColorStop(0,config.linearColor);
            grd.addColorStop(0.5, config.linearColor1);
            grd.addColorStop(1, config.linearColor2);
